<?php
    ...
    $_POST['acao'])) {
        $acao = // Dados enviados via formulário
        $_POST['acao'];
        ...
        if ($acao === 'alterar' && isset(// Dados enviados via formulário
    ...
    $_GET['excluir'])) {
        $idDel = (int)// Dados recebidos via URL
        $_GET['excluir'];
    ...
    if (isset(// Dados recebidos via URL
    $_GET['editar'])) {
        $edita_id = (int)// Dados recebidos via URL
        $_GET['editar'];
...
<input type="text" name="busca" class="form-control" placeholder="Nome, procedimento ou data (YYYY-MM-DD)" value="<?= htmlspecialchars(// Dados recebidos via URL
$_GET['busca'] ?? '') ?>" />
<button type="submit" class="btn btn-info">Buscar</button>
<?php if (!empty(// Dados recebidos via URL
$_GET['busca'])): ?>
